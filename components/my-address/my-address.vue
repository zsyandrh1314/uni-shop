...
<script>
  ...
  import { mapState, mapMutations, mapGetters } from 'vuex'
    ...
    methods:{
      ...
      // 选择收获地址
      async chooseAddress() {
        // 1. 调用小程序提供的 chooseAddress() 方法，即可使用选择收获地址的功能
        // 返回值是一个数组：第1项为错误对象； 第2项为成功之后的收获地址对象
      ...
      async reAuth() {
        // 提示用户对地址进行授权
        const [err2, confirmRequest] = await uni.showModal({
        ...
        // 3.4 如果用户点击了“取消”按钮，则提示用户“您取消了地址授权”
        if (confirmResult.cancel) return uni.$showMsg('您取消了地址授权！')
        
        if (confirmRequest.confirm) return uni.openSetting({
...
.address-info-box {
  ...
  .row2 {
    ...
    .row2-left {
      white-space: nowrap; // white-space 属性设置如何处理元素内的空白，nowrap为强制在同一行内显示所有文本，直到文本结束或者遭遇 br 对象。
    }
  }
}
</style>
